...
Configuration OM
{
    ...
    Node $AllNodes.NodeName
    {
        ...
        if(
            ($SystemCenter2016OperationsManagerDatabaseServer -eq $Node.NodeName) -or 
            ($SystemCenter2016OperationsManagerDatawarehouseServer -eq $Node.NodeName) -or
            ($SystemCenter2016OperationsManagerReportingServer -eq $Node.NodeName)
        )
        {
            foreach($SQLServer in $Node.SQLServers)
            {
                ...
                if(
                    (
                        ($SystemCenter2016OperationsManagerDatabaseServer -eq $Node.NodeName) -and
                        ($SystemCenter2016OperationsManagerDatabaseInstance -eq $SQLInstanceName)
                    ) -or 
                    (
                        ($SystemCenter2016OperationsManagerDatawarehouseServer -eq $Node.NodeName) -and
                        ($SystemCenter2016OperationsManagerDatawarehouseInstance -eq $SQLInstanceName)
                    ) -or
                    (
                        ($SystemCenter2016OperationsManagerReportingServer -eq $Node.NodeName) -and
                        ($SystemCenter2016OperationsManagerReportingInstance -eq $SQLInstanceName)
                    )
                )
                {
                    $Features += "SQLENGINE,"
        ...
        if ($SystemCenter2016OperationsManagerManagementServers[0] -eq $Node.NodeName)
        {
            ...
            xSCOMManagementServerSetup "OMMS"
            {
                ...
                DwSqlServerInstance = ($SystemCenter2016OperationsManagerDatawarehouseServer + "\" + $SystemCenter2016OperationsManagerDatawarehouseInstance)
            }
        }
